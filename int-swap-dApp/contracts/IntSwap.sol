...
contract IntSwap is Ownable{
   ...
   //global variables 
    ...
    function mintIntSwap ( uint _notional_amount, uint _escrow_amount, uint _swap_rate, uint _maturity_date, string _rate_type, address _counterparty) public { 
        start_date = now;
        // owner = msg.sender; 
        if(_rate_type == "Fixed to Variable"){
            fixed_to_var_owner = msg.sender;
            var_to_fixed_owner = _counterparty;
        }else{
            var_to_fixed_owner = msg.sender;
            fixed_to_var_owner = _counterparty;
        }
     ...
     modifier onlyVarToFixed() {
        require(msg.sender == var_to_fixed_owner,"Only variable to fixed party can call this.");
        _;
    }
    modifier onlyFixedToVar() {
        require(msg.sender == fixed_to_var_owner,"Only fixed to variable party can call this.");
        _;
    }
    function receiveVarToFixedEscrow(uint amount) public payable OnlyVarToFixed returns (bool VarToFixedEscrowFlag){ 
        ...
        if (VarToFixedEscrowRecd >= escrow_amount) {VarToFixedEscrowFlag = true;
        } 
        else {
    ...
    function receiveFixedToVarEscrow(uint amount) public payable OnlyFixedToVar returns (bool FixedToVarEscrowFlag){
        ...
        // need to send escrow back if over 48 hrs and cancel contract--not finished 
        // need to send excess back if over required escrow amount
        if (FixedToVarEscrowRecd >= escrow_amount) {
    ...
    function OKtoProceed() public returns (bool) {
        require (FixedToVarEscrowRecd >= escrow_amount);
        require (VarToFixedEscrowRecd >= escrow_amount);
