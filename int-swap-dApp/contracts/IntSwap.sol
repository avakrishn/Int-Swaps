...
contract IntSwap is Ownable{
   ...
   //global variables 
    ...
    function mintIntSwap ( uint _notional_amount, uint _escrow_amount, uint _swap_rate, uint _maturity_date, string _rate_type, address _counterparty) public { 
        ...
        // In solidity you cannot use the double equal sign to compare strings so you must use keccak256() function that generates a random number based on the string you pass to it. So if _rate_type is equal to "Fixed to Variable" then both number generated by keccak256 will be the same
