...
contract IntSwap is Ownable{
   ...
   //global variables 
    ...
    function mintIntSwap ( uint _notional_amount, uint _escrow_amount, uint _swap_rate, uint _maturity_date, string _swap_out_of_rate_type, address _counterparty) public { 
        contract_id++;
        uint _start_date = now;
        // owner = msg.sender; 
        address _fixed_to_var_owner;
        address _var_to_fixed_owner;
        // In solidity you cannot use the double equal sign to compare strings so you must use keccak256() function that generates a random number based on the string you pass to it. So if _swap_out_of_rate_type is equal to "fixed" then both number generated by keccak256 will be the same
        if(keccak256(_swap_out_of_rate_type) == keccak256("fixed")){
